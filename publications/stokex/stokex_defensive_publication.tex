...
Let each participant declare their own reference velocity, and write $\rho_i$ for its value
counted in $\alpha$ per unit of time. The exchange velocities become
\begin{equation}
    \dot{X}^\alpha_i = w_i\, \rho_i\, f(x_i)\,, \qquad
    \dot{X}^\beta_i = w_i\, \rho_i\, f(1/x_i)\, \priceBA_i\,,
    \label{eqOwnReference}
\end{equation}
which is \eqref{eqExchangeVelocities} with $w_i$ replaced by the \emph{effective weight}
\begin{equation}
    \hat{w}_i := w_i\, \rho_i \,. \label{eqEffectiveWeight}
\end{equation}
...
converted to $\alpha$ at a common price, so that $\rho_i = c$ for every participant. That
...
directions. The word ``convention'' in Sec.~\ref{secMarketPrice} understates it.
...
$\rho_i$ of their own pace. Every result above still holds, with $\hat{w}_i = w_i \rho_i$
throughout.

\textbf{What it costs.} If $\rho_i$ is free in magnitude, then $\theta_i$ is no longer the
...
\emph{binary} denomination, $\rho_i \in \{\dot{R},\ \dot{R}\priceAB_i\}$. In the reference
