...
It serves as a velocity unit, constant and common to all participants, defined in terms of asset A by convention.
...
The corresponding reference exchange velocity for asset B is $\dot{R} \priceBA_i$ in the eyes of participant $i$: with that conversion, the two lines of \eqref{eqExchangeSymmetry} are one and the same equation, read once with $\alpha$ and once with $\beta$.
The conversion factor is not a departure from the symmetry, it is what carries it --- and the symmetry is exact, not approximate: Annex~\ref{secReferenceGauge} shows that relabelling the two assets maps the mechanism onto itself, sending the market price to its reciprocal.
The form of the function controlling a participant's exchange velocities therefore satisfies principle number~4 (\emph{no bias between assets}) as an identity, not as a convention.
