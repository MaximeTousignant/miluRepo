...
\begin{equation}
    \hat{w}_i := w_i\, \dot{\rho}_i \,. \label{eqEffectiveWeight}
\end{equation}
...
nonnegative $\hat{w}_i$ whatsoever. A participant's own reference velocity is therefore
...
$\hat{w}_i \mapsto \hat{w}_i \priceBA_i$, and the closed form \eqref{eqMarketPrice} maps to
its own reciprocal:
\begin{equation*}
    \left( \frac{\displaystyle \sum_i \hat{w}_i \priceBA_i \cdot \priceBA_i}
                {\displaystyle \sum_i \hat{w}_i \priceBA_i \cdot \priceAB_i^{\,2}} \right)^{1/3}
    = \left( \frac{\displaystyle \sum_i \hat{w}_i \priceBA_i^{\,2}}
                  {\displaystyle \sum_i \hat{w}_i \priceAB_i} \right)^{1/3}
...
covariance demands the participant-dependent $\hat{w}_i \priceBA_i$. An impartial arbiter
...
$\beta$ \emph{at the same numerical magnitude} has $\hat{w}_i = w_i \priceAB_i$, which is
...
$\dot{\rho}_i$ of their own pace. Every result above still holds, with $\hat{w}_i = w_i \dot{\rho}_i$
throughout.
