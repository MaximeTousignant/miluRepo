...
Let each participant declare their own reference velocity, and write $\dot{\rho}_i$ for its value
counted in $\alpha$ per unit of time. The exchange velocities become
\begin{equation}
    \dot{X}^\alpha_i = w_i\, \dot{\rho}_i\, f(x_i)\,, \qquad
...
\textbf{Handing the pace to the participants.} What remains open is not symmetry but
\emph{scale}. The market may fix only a unit of time and let each participant declare a
third input alongside their estimate and their degree of confidence: the magnitude
$\dot{\rho}_i$ of their own pace. Every result above still holds, with $\dot{w}_i = w_i \dot{\rho}_i$
throughout.

\textbf{What it costs.} If $\dot{\rho}_i$ is free in magnitude, then $\theta_i$ is no longer the
only urgency dial: a participant can scale up their influence on the market price by
declaring a large reference velocity, without touching their degree of confidence. The
comparability of $\theta$ across participants is lost, and its bound $[0,100)\,\%$ stops
bounding anything. The conservative variant keeps both: a \emph{common} magnitude and a
\emph{binary} denomination, $\dot{\rho}_i \in \{\dot{R},\ \dot{R}\priceAB_i\}$. In the reference
implementation (Sec.~\ref{secReferenceImplementation}) the question does not arise:
$\dot{R}$ is the universal income rate of the t\^{o}k system, common to every participant
by construction, which is precisely what makes degrees of confidence comparable.
