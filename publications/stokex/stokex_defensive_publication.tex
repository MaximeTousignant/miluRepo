...
\textbf{The denomination is \emph{not} a gauge.} The second line of
\eqref{eqExchangeSymmetry} carries the factor $\dot{R}\priceBA_i$, which the text reads as
``the reference exchange velocity for asset B in the eyes of participant $i$''. That
factor is not a convention that could have been chosen otherwise: given the first line,
principle 2 (\emph{exchange at market price}) forces it, since
$-\dot{X}^\alpha_i / \priceAB_\Omega = w_i f(1/x_i) \dot{R} \priceBA_i$ by
\eqref{eqFunctionalEquation}. But it does mean that a market whose participants count
their pace in $\beta$ --- each converting at their own estimate, as that reading
prescribes --- has $\rho_i = \dot{R}_\beta \priceAB_i$, hence
$\hat{w}_i = w_i \priceAB_i$. That is a \emph{participant-dependent} factor, not a common
one, and by \eqref{eqEffectiveWeight} it moves the price:
\begin{equation*}
    \priceAB_\Omega \Big|_{\beta\text{-referenced}}
    = \left( \frac{\displaystyle \sum_i w_i \priceAB_i^{\,2}}{\displaystyle \sum_i w_i \priceBA_i} \right)^{1/3}
    \;\neq\;
    \left( \frac{\displaystyle \sum_i w_i \priceAB_i}{\displaystyle \sum_i w_i \priceBA_i^{\,2}} \right)^{1/3}
    = \priceAB_\Omega \Big|_{\alpha\text{-referenced}}
\end{equation*}
in general. Three participants suffice to see it: with estimates $(1, 4, 2)$ and weights
$(1, 1, \tfrac12)$, the $\alpha$-referenced market clears at $1.71595$ and the
$\beta$-referenced one at $2.33106$, a gap of $35.9\,\%$. (Converting instead at the
\emph{market} price would give a common factor $\priceAB_\Omega$, hence the same price
again --- but that is not what \eqref{eqExchangeSymmetry} says.)

\textbf{What principle 4 therefore asserts.} The swap $\alpha \leftrightarrow \beta$ does
not map the mechanism onto itself: it maps the $\alpha$-referenced \StokEx{} onto the
$\beta$-referenced one, and those are two different markets. What \emph{is} symmetric, and
what Fig.~\ref{figExchangeSymmetry} shows, is each participant's own pair of velocities:
the functional equation \eqref{eqFunctionalEquation} is exactly the statement that a
participant facing a market above their estimate behaves as one facing it below, with the
roles of the assets exchanged. Asset symmetry holds participant by participant; the choice
of the reference asset is a genuine decision, and it belongs to whoever sets up the market.

\textbf{Handing the choice to the participants.} That decision can be moved. Let the
market fix only a unit of time, and let each participant declare a third input alongside
their estimate and their degree of confidence: the asset in which their own pace is
counted, $\rho_i = \dot{R}$ or $\rho_i = \dot{R}\priceAB_i$. Every participant then sees
the fully symmetric pair \eqref{eqOwnReference}, and the residual asymmetry is no longer
imposed by the designer --- it is declared, like everything else the participant declares.
