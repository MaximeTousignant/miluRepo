...
|---|---|---|
| `traderF_slope_at_one` | la pente de `f` en `x = 1` vaut 3 — base de l'interprétation angulaire `w = tan(θ)/3` | 7.4 |
| `exchange_at_market_price` | tout échange se fait au prix du marché : `−Ẋᵅ/Ẋᵝ = [α/β]_Ω` | 7.5 |
| `market_clears` | le prix en forme close annule le flot net de l'actif A | 7.6 |
| `market_price_unique` | le prix d'équilibre est unique | 7.7 |
| `market_is_single_participant` | le marché entier se comporte comme un participant unique de poids `W_Ω` | 7.8 |
| `traderF_strictMonoOn` | la fonction de marchand est strictement croissante sur `(0, ∞)` — principe 7 | — |

Les numéros d'annexe sont ceux de la version d'août 2026 ; les clés `\label`
...
- `market_clears` démontre la **réciproque** de l'annexe 7.6. L'annexe part de
...
- `market_price_unique` démontre l'**unicité seule**. L'existence d'un prix
  d'équilibre, que l'annexe 7.7 obtient par le théorème des valeurs
