...
A note on the overdot: in this document it marks a \emph{flow}, an amount per unit of time (e.g. $\dot{X}$, $\dot{R}$), constant between two events of the market. It is not the derivative of a state variable: flows are inputs of the dynamics, and actual time derivatives are written explicitly, as in $dn_i/dt$.
...
The t\^{o}k is also the one relaxation of the regular-asset assumption: it carries a continuous demurrage at rate $k$, acting on the balances independently of the exchange velocities (consistently with principle 1: the \tokEx{} itself only transfers). Between two events, t\^{o}k balances obey $dn_i/dt = \dot{X}_i - k\, n_i$; the balance updates of the outer algorithm use the closed-form exponential solution, and the emptying time of a t\^{o}k account becomes $\frac{1}{k} \ln\left(1 - k\, n_i / \dot{X}_i\right)$, a strictly increasing function of \eqref{eqDeltaT}.
