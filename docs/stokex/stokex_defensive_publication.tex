...
Rather than asking each participant how much to buy or sell at every instant,
the \StokEx{} asks each participant two simple questions: how much do they think one asset is worth relative to the other, and how certain are they of that estimate?
From the answers, the \StokEx{} extracts two parameters for each participant $i$: their estimate $\priceAB_i$ of the relative value of assets A and B, and their degree of certainty $\theta_i$ in that estimate. Every participant thereby deploys their own personal trader robot $T_i$ instancitated from the rational trader robot function $T(\priceAB_i,\theta_i)$ with one standingwho ouputs the exchange (buy/sell) velocities for assets A and B.  much to buy , with one standing order: buy low and sell high, at every instant.

The degree of certainty is mapped to a weight through $w = \frac{1}{3}\tan\left(\pi\,\theta/200\,\%\right)$; geometrically, $\theta$ is the angle of the participant's response curve at its equilibrium point. Every participant exchanges continuously, at velocities given by a common trader function $f(x) = x^2 - 1/x$ of the ratio between the market price and the participant's estimate; both assets are treated symmetrically and all exchanges occur exactly at the market price. The market price is the unique positive price that simultaneously clears both assets; it has the closed form given in Eq.~\eqref{eqMarketPrice}.
...
\textbf{Keywords:} stokex, tok system, fair market, exchange algorithm, market mechanism, automated market maker (AMM), order book, decentralized exchange, continuous trading, market clearing, equilibrium price, currency exchange, time-weighted execution (TWAP).

\subsection*{Relation to known mechanisms}
The \StokEx{} is an automated exchange mechanism, but it differs from the known families of market designs. Unlike the continuous double auction (the order book of stock and cryptocurrency exchanges), it involves no orders, no bids or asks, and no counterparty matching. Unlike automated market makers (AMM) of decentralized exchanges --- constant-function market makers such as constant-product pools and their generalizations --- it holds no liquidity pool and derives no price from reserves. Unlike scoring-rule market makers such as Hanson's logarithmic market scoring rule (LMSR) for prediction markets~\cite{Hanson2003}, and the convex-cost-function market makers that generalize it~\cite{AbernethyChenVaughan2013}, it requires no subsidizing party to commit capital against a bounded worst-case loss, and it elicits a participant's belief directly, as a declared estimate and degree of certainty, rather than inferring it from trades against an accumulated position. In all these cases, the \StokEx{} price is instead computed in closed form from the participants' declared estimates and degrees of certainty, and the participants trade with each other, continuously, exactly at that price. The continuous execution spreads each transaction over time, as time-weighted average price (TWAP) execution strategies do on traditional markets, but here the property belongs to the market itself: supply and demand clear exactly at every instant.
