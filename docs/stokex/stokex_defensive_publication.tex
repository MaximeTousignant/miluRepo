...
\vspace{6pt}
\textit{Original technical memorandum: January 2022 (internal, undisclosed).\\ Published as a defensive publication: July 2026.}
\end{center}
...
Its publication is intended to preclude the grant of valid patent claims on these methods by any party. No exclusivity is sought by the publisher.
This document is distributed under the Creative Commons Attribution 4.0 International license (CC BY 4.0).
The \StokEx{} algorithm is part of the t\^{o}k system, an alternative economic system developed by Smoothop (\url{https://www.smoothop.org/}).

\subsection*{AI contribution statement}% locked
This document was prepared with the assistance of Milu, an AI agent (built on Anthropic's Claude models) integrated into the Smoothop project; Milu's own identity and protocol are public, at \url{https://github.com/MaximeTousignant/miluRepo} (tag \texttt{stokex-defpub-2026-07}).
Milu contributed to the mathematical formalization, to the machine-checked proofs (Lean~4 / mathlib) and numerical verification of the claims herein, and to the drafting of the text,
under the direction and review of the human author, who is solely responsible for its content.  % uman author, smooth operator

\subsection*{Name and pronunciation} % locked
The name \StokEx{} juxtaposes the two currencies of the exchange (the dollar and the t\^{o}k), followed by ``Ex'' for exchange. It is pronounced \textipa{[stOkEks]}: the dollar sign is read as an ``S''.
...
The \StokEx{} asks each participant two simple questions up front: how much do they think one asset is worth relative to the other, and how confident are they in that estimate?
From those answers, the \StokEx{} extracts two parameters for each participant identified by an index $i$: their estimate $\priceAB_i$ of the relative value of assets A and B, and their degree of confidence $\theta_i$, between 0 and 100\%, in that estimate.
Every participant thereby deploys their own personal trading robot with one standing order: buy low and sell high, at every instant.

The trading robot working for participant $i$ continuously outputs two exchange (buy/sell) velocities, one for asset A and one for asset B, that are coherent with its participant's standing order and with global market rules.
...
Every trading robot exchanges continuously at velocities $\dot{X}_i^\alpha$ and $\dot{X}_i^\beta$ proportional to its weight and to a common trader function $f(x_i) = x_i^2 - 1/x_i$,
...
From an outsider's perspective, the whole market is provably indistinguishable from a single participant with an aggregate estimate (equal to the market price) and an aggregate degree of confidence.
The state of the market is piecewise constant between events and is advanced exactly, event by event, by the two algorithms given below, with incremental $O(N \log N)$ updates.
Mathematical proofs are provided in the annexes.

\textbf{Keywords:} stokex, automated market maker (AMM), decentralized exchange, logarithmic market scoring rule (LMSR), order book, continuous trading, prediction market, tok system, time-weighted execution (TWAP).
...
Unlike automated market makers (AMM) of decentralized exchanges --- in particular constant-function market makers (CFMMs), such as Uniswap's constant-product pools~\cite{UniswapV2} and their generalizations~\cite{AngerisChitra2020} ---
the \StokEx{} holds no liquidity pool and derives no price from reserves.

Unlike scoring-rule market makers, such as Hanson's logarithmic market scoring rule (LMSR) for prediction markets~\cite{Hanson2003} and the convex-cost-function market makers that generalize it~\cite{AbernethyChenVaughan2013},
the \StokEx{} requires no subsidizing party to commit capital against a bounded worst-case loss.
Nothing prevents an ordinary participant from unilaterally anchoring the price near their own estimate by declaring a very high degree of confidence, but the role is not a privileged one: any participant may take it on, their resulting exposure is bounded by their own account balance rather than by a worst-case loss fixed in advance for the whole market by a designer, and the algorithm itself enforces solvency as that balance is spent.
Moreover, the \StokEx{} elicits a participant's belief directly, as a declared estimate and degree of confidence, rather than inferring it from trades against an accumulated position.

The \StokEx{} computes the market price in closed form from the participants' declared estimates and degrees of confidence, and the participants' robots trade with each other, continuously, exactly at that price.
The continuous execution spreads each transaction over time, as time-weighted average price (TWAP) execution strategies do on traditional markets, but here the property belongs to the market itself: supply and demand clear exactly at every instant.
...
It is not the derivative of a state variable. Actual time derivatives are written explicitly, as in $dn_i/dt$.

% ----------------------------------------------------------------------
\section{Principles of the \StokEx{} Algorithm}
% ----------------------------------------------------------------------

The purpose of the algorithm is to set up a market between different participants in which two fungible assets,
A and B, can be \textbf{continuously} exchanged according to each participant's estimate of the relative value of the two assets.

The goals that guide this market's design are fairness, transparency, and accessibility.
The \StokEx{} achieves accessibility by changing the question it asks a participant.
A traditional market asks \emph{how much do you want to buy or sell, now?} --- a question whose correct answer moves with every tick;
the \StokEx{} asks instead \emph{what do you think the two assets are worth relative to each other, and how sure are you?} --- a standing judgment that the mechanism then trades on continuously,
buying more when the market is cheap relative to the estimate and selling more when it is dear.
In effect, that standing judgment instantiates the participant's own personal trading robot, which trades on their behalf without further attention.

Accessibility also constrains the mechanism from the other side. A market that means to be accessible should ideally charge no fees; but a mechanism that collects no fees earns nothing to spend on running itself, so it cannot pass its computational cost on to anyone and must instead keep that cost as low as possible. This budget shapes the construction that follows: the state of the market is advanced exactly, event by event, in $O(N \log N)$ time, and wherever the form of a function is left free, the cheapest to evaluate is preferred --- one reason the trader function is taken at $p = 2$, $f(x) = x^2 - 1/x$, which is built from integer powers alone and calls no transcendental function (Annex~\ref{secTraderFamily}).

\textbf{Wallets.} Each participant holds a \emph{wallet}: one account per asset, with balances $n^\alpha_i$ and $n^\beta_i$.
Individual balances evolve continuously through the exchanges of the \StokEx{} (and through the participant's own deposits and withdrawals),
but the exchange only moves amounts between wallets: deposits and withdrawals apart, the \emph{sums} $\sum_i n^\alpha_i$ and $\sum_i n^\beta_i$ are constant.
Throughout this document, the assets are assumed to have no dynamics of their own; this assumption can be relaxed under certain conditions,
such as the continuous disintegration (sometimes called demurrage) of the t\^{o}k in the reference implementation (Sec.~\ref{secReferenceImplementation}), without touching the exchange itself.

The algorithm obeys eight fundamental principles, which fall into two classes.
The \emph{fair-market principles} (1--4) constrain the market itself, and the \emph{rational-trader principles} (5--8) specify the behavior of the participants ---
a behavior implemented for all participants by the same function, called the \emph{trader function}.
...
\begin{enumerate}
    ...
    \end{equation*}
    Since $\dot{X}_i$ is precisely the contribution of the exchange to the balance $n_i$, the same equation is a conservation law: the exchange creates and destroys nothing, it only transfers existing amounts between the participants' wallets.
    ...
    \end{equation*}
    The minus sign records that $\dot{X}_i^\alpha$ and $\dot{X}_i^\beta$ always carry opposite signs --- one asset comes in as the other goes out --- and they vanish together only at the participant's own estimate, where there is no trade.
    \item \emph{No bias between participants.} All exchange velocities are given by one common function of the participant's own inputs and of the market price, the same function for every participant:
    \begin{equation*}
...
\textbf{Rational-trader principles} (every participant trades rationally). Let $x_i := \priceAB_\Omega / \priceAB_i$ denote the ratio between the market price and participant $i$'s estimate:
\begin{enumerate}
    \setcounter{enumi}{4}
    ...
    \begin{equation*}
        \dot{X}_i \propto w_i = w(\theta_i)\,, \quad w \ \text{strictly increasing in } \theta_i\,.
    \end{equation*}
    No conviction, no trade: $w(0) = 0$.
\end{enumerate}

These eight principles are the \emph{hypotheses} of the \StokEx{}: everything that follows is constructed to satisfy them. The trader function \eqref{eqTraderFunction} realizes principles 5--8, the symmetric velocity pair \eqref{eqExchangeSymmetry} realizes principles 2--4, and the market price \eqref{eqMarketPrice} realizes principle 1.
...
Exchanges on the \StokEx{} are continuous, instead of discrete in time like on traditional markets. For example, rather than buying 5 units of $\beta$ in exchange for 10~units of $\alpha$ at some instant~$t$ on a traditional market (which implies a market price of 2~$\alpha$ per~$\beta$), the same exchange on the \StokEx{} could happen by continuously buying units of~$\beta$ (at the market price of 2~$\alpha$ per~$\beta$) with a velocity of 1~$\beta$ per hour for 5 hours.

On the participant level, the advantage of a continuous exchange is that, by spreading their transaction over a longer period of time, they reduce the risk of making their transaction at a bad time. As we will show, the \StokEx{} allows the participant to automatically either buy or sell more (i.e. at a higher velocity) when the market price is more advantageous for them.

At the market level, the advantage of a continuous exchange is that the market price would be more stable since every participant is automatically reacting to any change in the market price by increasing or decreasing their exchange velocities.

\subsection{The personal trading robot}
% ----------------------------------------------------------------------

Everything in this subsection is written from the point of view of a single participant, taking the market price $\priceAB_\Omega$ as a given input. We give the two exchange velocities the participant's robot outputs, the profit it accumulates over time, the moment it empties an account, and a worked example on real data. The market price it reacts to is itself constructed, from the aggregate of all participants' robots, in Sec.~\ref{secMarketPrice}.

\subsubsection{Exchange velocities}
% ----------------------------------------------------------------------

Every participant in the \StokEx{} has their exchange velocities --- the output of their personal trading robot (Fig.~\ref{figRobotSchema}) --- determined by the same function. In that sense, the \StokEx{} is not biased towards a participant or another and therefore fulfills principle number~3 mentioned earlier.
...
\end{equation}
The function $w$ is consistent with principle number 8 since it scales the participant's exchange velocities according to their degree of confidence (we show in Annex \ref{secProofDegree} that $\theta_i$ can be interpreted as a geometrical angle in the right context). This tangent map is a design choice, not a requirement: principle 8 only asks for a strictly increasing $w$ with $w(0) = 0$, and Annex~\ref{secWeightFamily} shows that every result in this document holds unchanged for any such function.
...
\end{equation}
Profit is therefore never independent of the asset it is quoted in: it is not that one of $P_i^\alpha$ and $P_i^\beta$ is ``the'' profit and the other a unit conversion of it, but that both are equally valid, related exactly by the market price of the moment. Because that price is always strictly positive (principle 2), $P_i^\alpha(t)$ and $P_i^\beta(t)$ always agree in sign --- a participant cannot be ahead in one asset and behind in the other --- but they generally disagree in magnitude, by exactly the factor $\priceAB_\Omega(t)$. This is the \StokEx{}'s asset symmetry (principle 4) carried into the derived notion of profit: no asset is privileged as the numeraire in which profit must be reported.


\subsubsection{Numerical illustration: a currency pair (CAD/EUR)}
% ----------------------------------------------------------------------

Before the formulas, a story. Alice holds dual Canadian and French citizenship, and she keeps two bank accounts: one in Canadian dollars (CAD), one in euros (EUR). Her life moves back and forth between the two countries, and so does her need for cash. Some months she is living in Lyon and suddenly needs euros; other months she is in Montreal and suddenly needs Canadian dollars. Most of the time, though, Alice is in no rush at all.

What Alice would like is easy to say and tedious to do by hand: when the Canadian dollar is cheap, she would like to buy it with her euros --- which is the very same thing as selling her euros while they are dear; and when the euro is cheap, she would like to buy it with her Canadian dollars --- selling her Canadian dollars while they are dear. In one phrase, Alice wants to \emph{buy low and sell high}, continuously, without watching the exchange rate all day.

This is exactly what the \StokEx{} does for her. Alice never has to place an order, pick a moment, or follow the market. She turns to the \StokEx{}, and it asks her just two questions (Fig.~\ref{figTikQuestions}): what does she think one currency is worth in terms of the other, and how sure is she of that number? From those two answers --- an estimate and a degree of confidence --- the \StokEx{} sets up her personal trading robot, which then trades gently and continuously on her behalf, leaning into whichever currency the market is offering cheaply at each instant. The rest of this section works out precisely what that robot would have done, over five years of real exchange-rate data.

\begin{figure}[htb]
\centering
\placeholder{[FIGURE \`A VENIR --- capture d'\'ecran du \StokEx{} dans l'impl\'ementation de r\'ef\'erence (syst\`eme des t\^{o}k), montrant les deux questions pos\'ees au participant (estimation et degr\'e de confiance), avec une monnaie \'etrang\`ere fictive, le TIK. Placeholder pour l'instant --- remplacer par l'image avant soumission.]}
\caption{The two questions the \StokEx{} asks a participant, shown in the reference implementation of the t\^{o}k system: an estimate of the relative value of the two assets, and a degree of confidence in it. Here the t\^{o}k is paired against a fictional foreign currency, the TIK.}
\label{figTikQuestions}
\end{figure}

There is one thing, though, that the \StokEx{} still needs and that Alice's two answers do not settle: how \emph{fast} her robot should trade overall --- the reference pace $\dot{R}$. In the t\^{o}k system this pace has a natural value, the system's own universal-income rate (Sec.~\ref{secReferenceImplementation}). Between two ordinary floating currencies there is no such anchor, and the reason is worth stating plainly: money is not a stable unit of value. The purchasing power of a Canadian dollar drifts downward over time, $d\,\mathrm{CAD}/dt < 0$, and so does that of a euro, each at its own shifting and unannounced rate, so neither provides a fixed clock against which a natural pace could be defined. For Alice we therefore simply pick one, and, as the next paragraphs make precise, that choice only rescales the amounts she trades, never the shape of what her robot does.

To make $\dot{R}$ tangible, take a round case --- distinct from the $\theta_i = 60\%$ Alice actually declares below, chosen only because the arithmetic comes out clean. Suppose a degree of confidence of exactly one half, $\theta_i = 50\%$; the weight is then $w = \tfrac{1}{3}\tan(\pi/4) = \tfrac{1}{3}$, and the robot's Canadian-dollar velocity is $\dot{X}^{\mathrm{CAD}} = \tfrac{1}{3}\, f(x)\, \dot{R}$, with $x$ the market rate divided by her estimate. Two mirror snapshots fix the scale. If the market ever valued the euro at \emph{twice} Alice's estimate ($x = 2$), the euro would be dear and her robot would sell euros, taking in Canadian dollars at $\tfrac{1}{3}\, f(2)\, \dot{R} = \tfrac{7}{6}\dot{R} \approx 1.17\,\dot{R}$. If the market valued it at \emph{half} ($x = \tfrac{1}{2}$), the euro would be cheap and her robot would buy euros, paying out Canadian dollars at $\tfrac{1}{3}\lvert f(\tfrac{1}{2})\rvert\, \dot{R} = \tfrac{7}{12}\dot{R} \approx 0.58\,\dot{R}$. So $\dot{R}$ is simply the pace of that flow: with the placeholder $\dot{R} = 1$~CAD/day used below, a market off by a factor of two moves on the order of one Canadian dollar through Alice's accounts per day, and doubling $\dot{R}$ would double every such amount. (The two Canadian-dollar figures differ only because both are read in the same currency; measured each in its own, buying and selling mirror one another exactly --- the symmetry of Fig.~\ref{figExchangeSymmetry}.)

To ground Fig.~\ref{figExchangeSymmetry} in a concrete case, we apply the exchange velocities to a real time series. To be precise about what this does and does not test: it exercises a single participant's personal trading robot, i.e.\ the exchange-velocity function of Eq.~\eqref{eqExchangeVelocities} alone, given a real price series taken as an exogenous input --- not the \StokEx{} algorithm as a whole. The market price itself, its equilibrium and uniqueness (Sec.~\ref{secMarketPrice}, Annex~\ref{secProofUniqueness}), and the aggregation of many participants into one (Sec.~\ref{secMarketBehavior}, Annex~\ref{secProofTotalMarketWeight}) are not exercised here: a real market running the \StokEx{} would need other participants, whose combined robots are what actually sets the price this one robot reacts to. The reference implementation of the \StokEx{} (Sec.~\ref{secReferenceImplementation}) exchanges the t\^{o}k against a foreign currency, so we illustrate this one instance on an actual currency pair, CAD against EUR, computed as the cross rate of two FRED series, \texttt{DEXCAUS}~\cite{FREDDEXCAUS} (CAD per USD) and \texttt{DEXUSEU}~\cite{FREDDEXUSEU} (USD per EUR). Two developed, floating currencies exchanging against each other is a far more stationary process than an equity index, with no comparable secular trend to extrapolate; a linear trend fit on this pair over five years explains very little of its variation ($R^2\approx0.17$, against commonly above $0.8$ for a rising stock index), so imposing one would not be a reasonable forecast. Instead, the declared estimate is the plain average exchange rate over the five years \emph{preceding} the test window, 2016-07-18 to 2021-07-16 --- still computed only from data that predates the test window, and still an honest out-of-sample estimate, just without assuming a trend the data does not support. This gives a declared estimate of $\approx1.497$~CAD per EUR, with a declared degree of confidence $\theta_i=60\%$ (hence $w_i\approx0.459$), against a realized average over the five-year validation window that follows, 2021-07-19 to 2026-07-17 (1250 trading days), of $\approx1.486$ --- a close, but not exact, match, again exactly as a real estimate would be. We then treat the real exchange rate over the validation window as the exogenous market price that this one robot reacts to, exactly as principle 2 requires of every participant regardless of what the others are doing.

Consider a participant who starts with 1000~CAD and 1000~EUR --- round, native amounts in each currency, with no conversion needed between them, since here \emph{both} assets are money. Money in each currency is kept in its own native unit throughout, rather than converted into a common yardstick: this is the \StokEx{}'s own asset symmetry (principle 4) carried into the example, and it is why the second panel of Fig.~\ref{figCADEURExample} uses two separate axes, color-coded blue for CAD and orange for EUR. One choice remains free: the reference exchange velocity $\dot{R}$, whose unit is left to the implementer (Sec.~\ref{secReferenceImplementation} ties it, in the reference implementation, to the t\^{o}k system's own universal income rate). No such external anchor exists for this illustrative retail portfolio, so we set $\dot{R}=1$~CAD/day, an arbitrary placeholder rather than an estimate of how actively a real participant would trade. Every CAD or EUR figure reported below --- cash flow, turnover, profit --- is exactly proportional to this choice and would rescale linearly with it; only the fraction of days spent buying versus selling, and the dates of the largest signals, do not depend on it. Because the estimate sits slightly above the eventual realized average, the exchange rate is below it more often than not: the participant's robot is a net buyer of EUR on 65.8\% of trading days and a net seller on the remaining 34.2\%, with the single largest buy signal on 2022-08-25 (rate at 1.289, the window's low) and the single largest sell signal on 2025-10-17 (rate at 1.640, the window's high). The CAD balance (Fig.~\ref{figCADEURExample}, second panel, blue) declines from 1000 to about 987, dipping as low as about 954 in February 2025, while the EUR balance (orange) rises from 1000 to about 1013, essentially never dipping below its starting point --- consistent with a robot that is, on net, mildly accumulating the currency it believes the market slightly underprices.

The last two panels apply Eqs.~\eqref{eqParticipantProfitAlpha}--\eqref{eqParticipantProfitBeta} to this participant, with CAD in the role of $\alpha$ and EUR in the role of $\beta$, taking $t_0$ as the start of the validation window:
\begin{align*}
P^{\mathrm{CAD}}(t) &= \left(n^{\mathrm{CAD}}(t) - n^{\mathrm{CAD}}(t_0)\right) + \left(n^{\mathrm{EUR}}(t) - n^{\mathrm{EUR}}(t_0)\right) [\mathrm{CAD/EUR}]_\Omega(t), \\
P^{\mathrm{EUR}}(t) &= \left(n^{\mathrm{EUR}}(t) - n^{\mathrm{EUR}}(t_0)\right) + \left(n^{\mathrm{CAD}}(t) - n^{\mathrm{CAD}}(t_0)\right) [\mathrm{EUR/CAD}]_\Omega(t).
\end{align*}
By Eq.~\eqref{eqProfitIdentity}, $P^{\mathrm{CAD}}(t) = P^{\mathrm{EUR}}(t)\, [\mathrm{CAD/EUR}]_\Omega(t)$ exactly: the two panels are not independent findings, only the same profit quoted twice, so they must and do agree in sign throughout --- there is no day on which this participant is ahead in one currency and behind in the other. What they do \emph{not} agree on is magnitude: priced in CAD (third panel), the participant ends about 7~CAD richer than at the start of the validation window, after a drawdown to about $-1$~CAD on 2022-08-25 and a peak of about 8~CAD on 2025-10-17. Priced in EUR (fourth panel), the very same holdings and the very same trades end about 5~EUR richer, with the same shape traced at a different scale, since 1~CAD then buys correspondingly less EUR than 1~EUR buys CAD. The lesson is not that the sign of profit is ambiguous, but that its \emph{size} is: ``how much richer is this participant'' is only a well-posed question once a currency is chosen to measure it in, and the \StokEx{} itself never makes that choice --- every principle in Sec.~\ref{secMarketPrice} is stated in terms of exchange velocities and ratios, never in terms of a profit computed in one privileged asset.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{cadeur_example.pdf}
\caption{Numerical illustration on real data: the CAD/EUR exchange rate (top, FRED series \texttt{DEXCAUS}~\cite{FREDDEXCAUS} and \texttt{DEXUSEU}~\cite{FREDDEXUSEU}, price axis anchored at zero) over the shaded calculation window, 2016-07-18 to 2021-07-16, used only to compute a plain average (solid, horizontal: no trend fit is warranted here) that becomes the declared estimate throughout the unshaded validation window, 2021-07-19 to 2026-07-17, $\approx1.497$~CAD per EUR (dashed line). Second: a participant starting with 1000~CAD (blue, left axis) and 1000~EUR (orange, right axis), each in its own native currency. Third and fourth: the same participant's profit since the start of the validation window (Eqs.~\eqref{eqParticipantProfitAlpha}--\eqref{eqParticipantProfitBeta}), accounted in CAD ($P^{\mathrm{CAD}}$, blue) and independently in EUR ($P^{\mathrm{EUR}}$, orange) --- the same quantity, related exactly by Eq.~\eqref{eqProfitIdentity}, not two independent findings.}
...
Starting from two sets of assumptions --- four \emph{fair-market} principles (conservation, a single strictly positive price common to all, and no bias between participants or between the two assets) and four \emph{rational-trader} principles (buy low, sell high, and trade the more urgently the wider the gap and the firmer the conviction) --- we were led to a mathematical framework for the continuous exchange of two fungible assets. The principles act as hypotheses that fix the shape of everything that follows: a trader function (here $f(x) = x^2 - 1/x$), a symmetric pair of exchange velocities, and a market price in closed form (Eq.~\eqref{eqMarketPrice}), unique and clearing both assets at every instant.

The framework has properties worth stating plainly.

It is \emph{fair}: unbiased between participants, and between the two assets.

It is \emph{transparent}: one price, common to all, computed in closed form from the declared inputs, with no privileged order flow.

It is \emph{accessible}, in two ways. It is free of fees: the exchange conserves the assets exactly and only transfers them between participants, with no intermediary skimming a share. And it is free of the need for constant monitoring: instead of asking how much to buy or sell at each instant, it asks what a participant thinks the two assets are worth relative to each other and how sure they are, a judgment they can set once and leave in place while the mechanism trades on it continuously.

Finally, the whole market is provably indistinguishable from a single aggregate participant (Annex~\ref{secProofTotalMarketWeight}), and its state advances exactly, event by event, in $O(N \log N)$. The central results are machine-checked in Lean~4 / mathlib; the \StokEx{}, together with the whole admissible family of trader functions (Annex~\ref{secTraderFamily}), is disclosed here as prior art.
...
v4.32.0). Insérer ici l'URL figée au tag \texttt{stokex-defpub-2026-07} :
\url{https://github.com/MaximeTousignant/miluRepo/tree/stokex-defpub-2026-07/docs/stokex/proof}}
...
\textbf{What the principles force.} Let $f$ be any candidate trader function, used symmetrically by both assets as in \eqref{eqExchangeSymmetry}, and write $x = \priceAB_\Omega / \priceAB_i$. The two exchange velocities of participant $i$ are then $\dot{X}^\alpha_i = w_i f(x)\, \dot{R}$ and $\dot{X}^\beta_i = w_i f(1/x)\, \dot{R} \priceBA_i$. Principle 2 requires $-\dot{X}^\alpha_i / \dot{X}^\beta_i = \priceAB_\Omega = x \priceAB_i$ for every market state, which simplifies to the functional equation
\begin{equation}
...
\end{equation*}
in which $\theta_i$ is the participant's ``degree of confidence'' contained between 0\% and 100\%.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{degree_of_certainty_log.pdf}
    \caption{Geometrical interpretation of the participant's degree of confidence $\theta_i$ (logarithmic scale in $x$)}
    \label{figDegreeLog}
\end{figure}
\clearpage
