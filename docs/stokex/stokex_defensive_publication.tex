...
This document was prepared with the assistance of Milu,
an AI agent (built on Anthropic's Claude models) integrated into the Smoothop project;
Milu's own identity and protocol are public,
at \url{https://github.com/MaximeTousignant/miluRepo} (tag \texttt{stokex-defpub-2026-07}).
Milu contributed to the mathematical formalization, to the machine-checked proofs (Lean~4 / mathlib)
and numerical verification of the claims herein, and to the drafting of the text,
...
\textbf{Keywords:} stokex, automated market maker (AMM), decentralized exchange, logarithmic market scoring rule (LMSR),
order book, continuous trading, prediction market, tok system, time-weighted execution (TWAP).
...
of the relative value of the two assets.
\emph{Continuous} here is meant in time:
rather than buying 5~$\beta$ for 10~$\alpha$ at a single instant
...
A traditional market asks \emph{how much do you want to buy or sell, now?}
...
--- a standing judgment that the mechanism then trades on continuously,
...
The symmetric velocity pair \eqref{eqExchangeSymmetry} realizes principles~2--4,
...
We give the two exchange velocities the participant's trading robot outputs, the profit it accumulates over time, the moment it empties an account, and a worked example on real data.
...
In that sense, the \StokEx{} is not biased towards a participant or another and therefore fulfills principle number~3 mentioned earlier.
...
\end{align}
where $\dot{R}$ is the reference exchange velocity (in $\alpha$ / unit of time) for this particular market.
The algorithm requires that a reference exchange velocity, constant and common to all participants, be defined for one of the two assets.
...
By convention, the reference exchange velocity $\dot{R}$ is defined in terms of asset A. The corresponding reference exchange velocity for asset B is $\dot{R} \priceBA_i$ in the eyes of participant $i$.
Apart from this difference, the two equations in \eqref{eqExchangeSymmetry} are perfectly symmetric in the sense that they are the same except that $\alpha$ is swapped with $\beta$.
This property ensures that the market is not biased towards one asset or the other. Therefore, the form of the function controlling a participant's exchange velocities satisfies principle number~4.

Next, we need to specify the functions~$w$ and~$f$ that compose the exchange velocities function.
...
This function describes the general behavior of a participant's trading robot, depending on the ratio between the market price and the participant's estimate~--- either $\priceAB_\Omega / \priceAB_i$ or $\priceBA_\Omega / \priceBA_i$.
This general behavior follows principles number~5, 6 and~7. The \StokEx{} satisfies these principles by defining the trader function, for any price ratio $x > 0$, as
\begin{equation}
    f(x) = x^2 - \frac{1}{x}\,. \label{eqTraderFunction}
\end{equation}
This choice is principled but not arbitrary. As shown in Annex~\ref{secTraderFamily}, principles 2 and 4 by themselves force two structural properties of any admissible trader function:
...
\end{equation}
The function $w$ is consistent with principle number 8 since it scales the participant's exchange velocities according to their degree of confidence (we show in Annex \ref{secProofDegree} that $\theta_i$ can be interpreted as a geometrical angle in the right context).
This tangent map is a design choice, not a requirement: principle 8 only asks for a strictly increasing $w$ with $w(0) = 0$, and Annex~\ref{secWeightFamily} shows that every result in this document holds unchanged for any such function.
Either way, what the present document discloses is the per-participant weighting itself --- the scaling of each participant's exchange velocities by a weight $w_i \geq 0$ --- independently of the variable substitution used to derive that weight from a declared degree of confidence or equivalent conviction signal, of which the tangent map \eqref{eqFunctionW} is only one instance.

The participant's weight $w_i$ is computed only once from their declared degree of confidence and, until the participant revises it, need not be recomputed at any step of the algorithm.
...
\end{align}
Because the two velocities always carry opposite signs (principle~2), at most one of $\Delta t_i^\alpha$ and $\Delta t_i^\beta$ is finite --- that of the asset currently being sold. The participant's emptying time is therefore
\begin{equation}
...
Before the formulas, a story. Alice holds dual Canadian and French citizenship and keeps two accounts, one in Canadian dollars (CAD), one in euros (EUR). Her life --- and her need for cash --- moves between Montreal and Lyon, but most of the time she is in no rush. What she wants is easy to say and tedious to do by hand: buy each currency while it is cheap and sell it while it is dear, continuously, without watching the rate all day. This is exactly what the \StokEx{} does for her: she answers just two questions --- what she thinks one currency is worth in the other, and how sure she is --- and from that estimate and degree of confidence it sets up her personal trading robot, which trades gently and continuously on her behalf. The rest of this section works out what that robot would have done over five years of real exchange-rate data.

One input her two answers do not settle is how \emph{fast} the robot should trade overall --- the reference pace $\dot{R}$. In the t\^{o}k system this has a natural value, the system's universal-income rate (Sec.~\ref{secReferenceImplementation}); between two floating currencies there is no such anchor, because money is not a stable unit of value --- the purchasing power of each drifts downward, $d\,\mathrm{CAD}/dt < 0$, at its own unannounced rate. We therefore simply set $\dot{R} = 1$~CAD/day: an arbitrary placeholder, not an estimate of how actively a real participant trades. Every CAD or EUR amount below is exactly proportional to it; only the \emph{fractions} of days spent buying versus selling, and the dates of the largest signals, do not depend on it.

What follows exercises a single participant's robot --- the exchange-velocity function \eqref{eqExchangeVelocities} alone, fed a real price series as an exogenous input --- not the full \StokEx{}: the market price, its uniqueness (Annex~\ref{secProofUniqueness}) and the aggregation of many participants (Sec.~\ref{secMarketBehavior}) are not exercised here, since a real market would need the other participants whose robots actually set the price this one reacts to. We take CAD against EUR, the cross rate of two FRED series, \texttt{DEXCAUS}~\cite{FREDDEXCAUS} (CAD per USD) and \texttt{DEXUSEU}~\cite{FREDDEXUSEU} (USD per EUR). Two developed floating currencies form a far more stationary pair than an equity index --- a linear trend explains almost none of the variation ($R^2\approx0.17$, against above $0.8$ for a rising stock index) --- so the declared estimate is simply the average rate over the five years \emph{preceding} the test window (2016-07-18 to 2021-07-16), an honest out-of-sample value of $\approx1.497$~CAD per EUR. Alice declares a degree of confidence $\theta_i=70\%$ (hence $w_i\approx0.654$). Over the following validation window (2021-07-19 to 2026-07-17, 1250 trading days) the rate averages $\approx1.486$ --- close to her estimate, but not exact, as a real one would be. We treat that real exchange rate as the exogenous market price this robot reacts to, exactly as principle 2 requires of every participant regardless of what the others are doing.

Alice starts with 100~CAD and 100~EUR --- small, round, native amounts, each kept in its own unit (the asset symmetry of principle 4 carried into the example, and why the second panel of Fig.~\ref{figCADEURExample} uses two separate axes, blue for CAD and orange for EUR). Because her estimate sits just above the realized average, the rate is below it more often than not: her robot is a net buyer of EUR on 65.8\% of trading days and a net seller on the remaining 34.2\%, with its single largest buy on 2022-08-25 (rate 1.289, the window's low) and its single largest sell on 2025-10-17 (rate 1.640, the window's high). The CAD balance (Fig.~\ref{figCADEURExample}, second panel, blue) falls from 100 to about 81, dipping to about 35 in February 2025 before recovering, while the EUR balance (orange) rises from 100 to about 118, essentially never below its starting point --- a robot mildly accumulating the currency it believes the market slightly underprices.
...
\end{align*}
By Eq.~\eqref{eqProfitIdentity}, $P^{\mathrm{CAD}}(t) = P^{\mathrm{EUR}}(t)\, [\mathrm{CAD/EUR}]_\Omega(t)$ exactly: the two panels are the same profit quoted twice, agreeing in sign throughout --- there is no day on which Alice is ahead in one currency and behind in the other --- but differing in magnitude. Priced in CAD (third panel) she ends about 10.3~CAD richer than at the start of the validation window, after a shallow drawdown to about $-1.9$~CAD on 2022-08-25 and a peak of about 11.4~CAD on 2025-10-17; priced in EUR (fourth panel), the very same trades end about 6.4~EUR richer. Against her starting wealth of $100 + 100 \times [\mathrm{CAD/EUR}]_\Omega(t_0) \approx 250$~CAD, that is a five-year return of about 4\% (4.1\% counted in CAD, 3.9\% in EUR). Modest --- but this is a currency pair, not the stock market: two developed floating currencies oscillate within a narrow band, with none of the secular climb a trending equity index would hand you. The point is not the size of the gain but its \emph{ambiguity}: ``how much richer is Alice'' is only a well-posed question once a currency is chosen to measure it in, and the \StokEx{} itself never makes that choice --- every principle in Sec.~\ref{secMarketPrice} is stated in terms of exchange velocities and ratios, never in terms of a profit computed in one privileged asset.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{cadeur_example.pdf}
\caption{Numerical illustration on real data: the CAD/EUR exchange rate (top, FRED series \texttt{DEXCAUS}~\cite{FREDDEXCAUS} and \texttt{DEXUSEU}~\cite{FREDDEXUSEU}, price axis anchored at zero) over the shaded calculation window, 2016-07-18 to 2021-07-16, used only to compute a plain average (solid, horizontal: no trend fit is warranted here) that becomes the declared estimate throughout the unshaded validation window, 2021-07-19 to 2026-07-17, $\approx1.497$~CAD per EUR (dashed line). Second: a participant starting with 100~CAD (blue, left axis) and 100~EUR (orange, right axis), each in its own native currency. Third and fourth: the same participant's profit since the start of the validation window (Eqs.~\eqref{eqParticipantProfitAlpha}--\eqref{eqParticipantProfitBeta}), accounted in CAD ($P^{\mathrm{CAD}}$, blue) and independently in EUR ($P^{\mathrm{EUR}}$, orange) --- the same quantity, related exactly by Eq.~\eqref{eqProfitIdentity}, not two independent findings.}
...
\textbf{What the principles force.} Let $f$ be any candidate trader function, used symmetrically by both assets as in \eqref{eqExchangeSymmetry}, and write $x_i = \priceAB_\Omega / \priceAB_i$. The two exchange velocities of participant $i$ are then $\dot{X}^\alpha_i = w_i f(x_i)\, \dot{R}$ and $\dot{X}^\beta_i = w_i f(1/x_i)\, \dot{R} \priceBA_i$. Principle 2 requires $-\dot{X}^\alpha_i / \dot{X}^\beta_i = \priceAB_\Omega = x_i \priceAB_i$ for every market state, which simplifies to $f(1/x_i) = -f(x_i)/x_i$. Since $x_i$ ranges over all of $(0,\infty)$ as the market state varies, $f$ must satisfy the functional equation
\begin{equation}
...
\end{equation*}
a finite limit, and the asset-B velocity follows identically. The product does not blow up because $f(x) = 3\,(x-1) + O\!\left((x-1)^2\right)$ near $x = 1$ (recall $f(1) = 0$ and $f'(1) = 3$), so $f\!\left(\priceAB_\Omega / \priceAB_\ell\right)$ vanishes at exactly the $1/w_\ell$ rate at which $\priceAB_\Omega$ approaches $\priceAB_\ell$.
