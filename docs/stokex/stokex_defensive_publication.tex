...
The pseudocode above is written for regular assets: the balance-update lines $n_i \gets n_i + \dot{X}_i\, \Delta t$ integrate $dn_i/dt = \dot{X}_i$ exactly. For an asset with intrinsic dynamics, these lines and the $\Delta t_i$ of the inner algorithm are replaced by the closed-form solution of the corresponding balance equation (for a demurrage, the exponential behind \eqref{eqDeltaTDemurrage}); the event-driven structure of both algorithms is unchanged, and the stepping remains exact.
