...
For an asset with a continuous exponential demurrage at rate $k$ (such as the t\^{o}k of the reference implementation), the balance obeys $dn_i/dt = \dot{X}_i - k\, n_i$, whose solution reaches zero after
...
The pseudocode above is written for regular assets, whose balances change only through the exchange: between two events, $dn_i/dt = \dot{X}_i$, so the update $n_i \gets n_i + \dot{X}_i\, \Delta t$ is exact. If an asset has intrinsic dynamics, exactly two things change:
\begin{itemize}
    \item the balance updates integrate the full balance equation over the step; for an asset with demurrage at rate $k$, solving $dn_i/dt = \dot{X}_i - k\, n_i$ over $\Delta t$ gives
    \begin{equation*}
        n_i \gets n_i\, e^{-k \Delta t} + \frac{\dot{X}_i}{k} \left(1 - e^{-k \Delta t}\right);
    \end{equation*}
    \item the emptying times $\Delta t_i$ computed by the inner algorithm use the matching formula, \eqref{eqDeltaTDemurrage} instead of \eqref{eqDeltaT}.
\end{itemize}
The event-driven structure of both algorithms is unchanged, and the stepping remains exact: in both cases the update is the closed-form solution of the balance equation, not a numerical approximation.
