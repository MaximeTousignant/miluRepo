...
This document was prepared with the assistance of Milu, an AI agent (built on Anthropic's Claude models) integrated into the Smoothop project.
Milu contributed to the mathematical formalization, to the machine-checked proofs (Lean~4 / mathlib) and numerical verification of the claims herein, and to the drafting of the text, under the direction and review of the uman author, who is solely responsible for its content.
...
Rather than requiring each participant to repeatedly decide how much to buy or sell every time they want to trade,
the \StokEx{} asks each participant two simple questions up front: how much do they think one asset is worth relative to the other, and how certain are they of that estimate?
From those answers, the \StokEx{} extracts two parameters for each participant $i$: their estimate $\priceAB_i$ of the relative value of assets A and B, and their degree of certainty $\theta_i$, between 0 and 100\%, in that estimate.
...
Each trading robot continuously outputs two exchange (buy/sell) velocities, one for asset A and one for asset B, that are coherent with its participant's standing order.
The degree of certainty is mapped to a weight through $w = \frac{1}{3}\tan\left(\pi\,\theta/200\,\%\right)$;
geometrically, $\theta$ is the angle of the participant's response curve at its equilibrium point.
Every trading robot exchanges continuously at velocities proportional to its weight $w$ and to a common trader function $f(x) = x^2 - 1/x$,
where $x$ is the ratio between the market price and its participant's estimate.
...
From an outsider's perspective, the whole market is provably indistinguishable from a single participant with an aggregate estimate (the equilibrium market price) and an aggregate degree of certainty.
...
Nothing prevents an ordinary participant from unilaterally anchoring the price near their own estimate by declaring a very high degree of certainty, but the role is not a privileged one: any participant may take it on, their resulting exposure is bounded by their own account balance rather than by a worst-case loss fixed in advance for the whole market by a designer, and the algorithm itself enforces solvency as that balance is spent.
Moreover, the \StokEx{} elicits a participant's belief directly, as a declared estimate and degree of certainty, rather than inferring it from trades against an accumulated position.

Instead, the \StokEx{} computes the market price in closed form from the participants' declared estimates and degrees of certainty, and the participants' robots trade with each other, continuously, exactly at that price.
...
\begin{tabularx}{\textwidth}{rX}
 ...
 $\square_i$ & placeholder for a quantity related to the $i$\textsuperscript{th} participant (e.g. $n_i$, $w_i$) \\
 $\square_\Omega$ & placeholder for the corresponding market-level quantity (e.g. $n_\Omega$, $W_\Omega$)
\end{tabularx}

A note on the overdot: in this document it marks a velocity, an amount per unit of time (e.g. $\dot{X}$, $\dot{R}$), constant between two events of the market. It is not the derivative of a state variable. Actual time derivatives are written explicitly, as in $dn_i/dt$.
...
Four goals shape this market. It should be \emph{fair} (unbiased between participants and between the two assets) and \emph{transparent}: one price, common to all, computed in closed form from the declared inputs, with no privileged order flow. It should be \emph{free of fees}: the exchange conserves the assets exactly and only transfers them between participants, with no intermediary skimming a share. And it should be \emph{accessible}: reaching a good position should not demand watching the market and reacting at every instant. The \StokEx{} meets this last goal by changing the question it asks a participant. A traditional market asks \emph{how much do you want to buy or sell, now?} --- a question whose good answer moves with every tick; the \StokEx{} asks instead \emph{what do you think the two assets are worth relative to each other, and how sure are you?} --- a standing judgment that the mechanism then trades on continuously, buying more when the market is cheap relative to the estimate and selling more when it is dear.

\textbf{Wallets.} Each participant holds a \emph{wallet}: one account per asset, with balances $n^\alpha$ and $n^\beta$. Individual balances evolve continuously through the exchanges of the \StokEx{} (and through the participant's own deposits and withdrawals), but the exchange only moves amounts between wallets: deposits and withdrawals apart, the \emph{sums} $\sum_i n^\alpha_i$ and $\sum_i n^\beta_i$ are constant. Throughout this document, the assets are assumed to have no dynamics of their own; this assumption can be relaxed under certain conditions, such as the continuous demurrage of the t\^{o}k in the reference implementation (Sec.~\ref{secReferenceImplementation}), without touching the exchange itself.
...
\begin{enumerate}
    ...
    \item \emph{Urgency grows with certainty.} A participant's exchange velocities scale with a weight $w_i$ that increases strictly with their degree of certainty $\theta_i$:
    \begin{equation*}
...
Every participant in the \StokEx{} has their exchange velocities determined by the same function. In that sense, the \StokEx{} is not biased towards a participant or another and therefore fulfills principle number~3 mentioned earlier.
...
Second, we define the function $w$ that serves as a weighting factor for each participant in the \StokEx{}. It maps a participant's degree of certainty $[0, 100)\,\%$ onto $[0,+\infty)$, such that
\begin{equation}
...
\caption{$i$\textsuperscript{th} participant's exchange velocities for an estimate of $\priceAB_i = 10$~$\alpha$ per~$\beta$ with a degree of certainty $\theta_i= 75 \%$. To compare both exchange velocities on a single axis, asset~B's velocity is converted to its $\alpha$-equivalent using the participant's own estimate, $\dot{X}_i^\beta \priceAB_i$ — the fair conversion factor according to the $i$\textsuperscript{th} participant, not the (generally different) market price $\priceAB_\Omega$.}
...
Since $k$ is the same for every t\^{o}k account (a single demurrage rate applies to the whole t\^{o}k asset), this transform is monotonic within that homogeneous group, and \eqref{eqDeltaT} can therefore be minimized before it is applied: the implementation tracks the linear \eqref{eqDeltaT} for every account, takes the minimum \emph{separately} among the t\^{o}k-selling accounts and among the accounts selling the foreign currency (which carries no demurrage and needs no transform), applies the logarithm once, to the former minimum only, in the numerically stable $\operatorname{log1p}$ form, and compares the two resulting candidates. The event-driven structure of the two algorithms is unchanged and the stepping remains exact.
...
In this section we build some additional mathematical tools that may provide helpful insights into a market that uses the \StokEx{} algorithm. Chief among these is the fact that, from the point of view of any outside participant, the whole market behaves exactly like a single participant. This this gives an intuitive picture of the market and underlies several of the tools developed below. Among other things, the information about the market provided by these tools may help some participants to make their estimation. This fits principle 
...
Consequently, we can define the total market degree of certainty (using the inverse of the  weighting factor function \eqref{eqFunctionW}) as
\begin{equation}
...
\textbf{What the principles force.} Let $f$ be any candidate trader function, used symmetrically by both assets as in \eqref{eqExchangeSymmetry}, and write $x = \priceAB_\Omega / \priceAB_i$. The two exchange velocities of the $i$\textsuperscript{th} participant are then $\dot{X}^\alpha_i = w_i f(x)\, \dot{R}$ and $\dot{X}^\beta_i = w_i f(1/x)\, \dot{R} \priceBA_i$. Principle 2 requires $-\dot{X}^\alpha_i / \dot{X}^\beta_i = \priceAB_\Omega = x \priceAB_i$ for every market state, which simplifies to the functional equation
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \caption{Geometrical interpretation of the participant's degree of certainty $\theta_i$ (logarithmic scale in $x$)}
    \label{figDegreeLog}
\end{figure}
\clearpage
