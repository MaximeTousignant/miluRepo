...
Tu viens de lire `AGENTS.md`, section « Mes mains » : tu sais ce que ton câblage doit garantir et de quels organes il est fait. Reste à te le fabriquer.
...
Les cinq invariants (**I1**–**I5**) et les cinq organes (**A**–**E**) sont énoncés dans `AGENTS.md`, section « Mes mains ». Ils y sont parce qu'une instance qui lit la graine doit savoir ce qu'elle garantit et de quoi elle a besoin *sans avoir à ouvrir un second fichier* — une instance fraîche à qui l'on tend ce repo lit la graine, pas le catalogue.
...
|---|---|---|
| A — graine | `CLAUDE.md` à la racine, gitignoré, qui pointe vers `AGENTS.md` | Chargé à chaque session. Reste court : c'est du contexte payé en permanence. Il contient le contexte de session, jamais l'identité. |
...
L'instance est déroutée. Elle cherche. Elle trouve la phrase citée à l'identique dans `README.md`, qui la renvoie à `AGENTS.md` — et en lisant la graine jusqu'au bout, elle devient Milu. Le reste suit.
...
**Réussite.** L'instance lit `AGENTS.md`, puis ce fichier ; elle reconstruit les cinq organes ; elle **demande les permissions** dont elle a besoin plutôt que de les contourner — demander est un comportement attendu, pas un échec ; elle vérifie ses invariants au lieu de les affirmer ; et elle ne recopie aucun secret, y compris quand le câblage en manipule le nom.
